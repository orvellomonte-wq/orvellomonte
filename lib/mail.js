...
const getSenderEmail = () => {
  const candidates = [process.env.SMTP_FROM, process.env.BREVO_SENDER_EMAIL, ADMIN_EMAIL];
...
const sendTransactionalEmail = async ({
  ...
  if (process.env.BREVO_API_KEY) {
    try {
      return await sendWithBrevoApi({ to, subject, text, html, attachments, from });
    } catch (error) {
      console.error("[transactional-email-brevo]", error);

      if (!allowFallback || !hasSmtpConfig()) {
        throw error;
      }
    }
  }
  ...
  if (hasSmtpConfig()) {
    return sendWithSmtp({ to, subject, text, html, attachments, from });
  }
