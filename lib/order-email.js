...
const buildPaidOrderEmail = (merchantOid, order) => {
  ...
  const customerName = String(order.customer?.fullName || "Musterimiz").trim();
  ...
  const discountLine = order.discount?.amount
    ? `<tr><td style="padding:5px 0;color:#555">Indirim (${escapeHtml(order.discount.code || "Kod")})</td><td style="padding:5px 0;text-align:right;color:#a52b1d">-${formatMoney(order.discount.amount)}</td></tr>`
    : "";

  const subject = `Siparisiniz alindi - ${merchantOid}`;
  const text = [
    `Merhaba ${customerName},`,
    "",
    "Odemeniz onaylandi ve satin alma isleminiz tarafimiza ulasti.",
    `Siparis numaraniz: ${merchantOid}`,
    "",
    textItems,
    "",
    `Ara toplam: ${formatMoney(order.subtotal)}`,
    order.discount?.amount ? `Indirim: -${formatMoney(order.discount.amount)}` : "",
    `Kargo: ${Number(order.shipping?.amount || 0) === 0 ? "Ucretsiz" : formatMoney(order.shipping.amount)}`,
    `Toplam: ${formatMoney(order.total)}`,
    "",
    "PDF siparis ve odeme ozetiniz bu e-postaya eklenmistir. Bu belge resmi e-Arsiv faturasi yerine gecmez; resmi faturaniz ayni e-posta adresine ayrica gonderilecektir.",
    "Degisim icin Instagram uzerinden @orvellomonte hesabimiza ulasabilirsiniz:",
    INSTAGRAM_URL,
    "",
    "Orvello Monte"
  ].filter(Boolean).join("\n");
  ...
  <body style="margin:0;background:#ecebe7;font-family:Arial,sans-serif;color:#111">
    <div style="max-width:680px;margin:0 auto;padding:28px 14px">
      ...
      <div style="background:#fff;padding:30px 24px">
        <p style="margin:0 0 8px;color:#a52b1d;font-size:12px;font-weight:800;letter-spacing:1.5px">ODEME ONAYLANDI</p>
        <h1 style="margin:0 0 16px;font-size:30px;line-height:1.1">Siparisiniz bize ulasti.</h1>
        <p style="margin:0 0 22px;color:#555;line-height:1.65">Merhaba ${escapeHtml(customerName)}, odemeniz basariyla onaylandi. Siparisinizi hazirlamaya basliyoruz.</p>
        <div style="padding:16px;background:#f2f1ed;border-left:4px solid #a52b1d">
          <span style="display:block;color:#666;font-size:11px;letter-spacing:1px">SIPARIS NUMARASI</span>
        ...
        <table role="presentation" style="width:100%;margin-top:18px;border-collapse:collapse;font-size:14px">
          <tbody>
            ...
            ${discountLine}
            <tr><td style="padding:5px 0;color:#555">Kargo</td><td style="padding:5px 0;text-align:right">${Number(order.shipping?.amount || 0) === 0 ? "Ucretsiz" : formatMoney(order.shipping.amount)}</td></tr>
        ...
        <div style="margin-top:26px;padding-top:22px;border-top:1px solid #dedbd4;color:#555;font-size:13px;line-height:1.65">
          ...
          PDF siparis ve odeme ozetiniz bu e-postaya eklenmistir. Bu belge resmi e-Arsiv faturasi yerine gecmez; resmi e-Arsiv faturasi ayni e-posta adresine ayrica gonderilecektir.
        </div>
        <div style="margin-top:18px;padding:18px;background:#111;color:#fff;font-size:13px;line-height:1.6">
          Degisim icin Instagram uzerinden bize ulasabilirsiniz.<br>
