...
const buildPaidOrderEmail = (merchantOid, order) => {
  ...
  const discountLine = order.discount?.amount
    ...
    : "";
  const campaignLine = order.campaign?.amount
    ? `<tr><td style="padding:5px 0;color:#555">2 Al 1 Hediye</td><td style="padding:5px 0;text-align:right;color:#397a32">-${formatMoney(order.campaign.amount)}</td></tr>`
    : "";
  ...
  const text = [
    ...
    order.campaign?.amount ? `2 Al 1 Hediye: -${formatMoney(order.campaign.amount)}` : "",
